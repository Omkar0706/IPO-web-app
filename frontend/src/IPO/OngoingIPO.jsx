...
const OngoingIPO = () => {
  ...
  // ✅ Fetch ongoing IPOs from backend
  useEffect(() => {
    const fetchOngoingIPOs = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/api/ipo/');
        const data = await response.json();
        const filtered = data.filter(ipo => ipo.status === 'ongoing');
        setOngoingIPOs(filtered);
      } catch (error) {
  ...
  return (
    <div className="relative mt-14 px-4 sm:px-6 lg:px-8">
      ...
      {ongoingIPOs.length === 0 ? (
        <p className="text-gray-500">No ongoing IPOs found.</p>
      ) : (
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-sm">{formatDate(ipo.open_date)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">CLOSE</p>
                      <p className="font-medium text-sm">{formatDate(ipo.close_date)}</p>
                    </div>
                  </div>
